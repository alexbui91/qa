...
def main(restore=False):
    ...
    word_embedding_file = 'sentiment/word_embedding.pkl'
    ...
    # init word embedding
    contexts, contexts_len, pred = utils.load_file('sentiment/sentiment_chunking.pkl')

    data_len = int(np.floor(0.9 * len(contexts))) 
    
    train = contexts[:data_len], contexts_len[:data_len], pred[:data_len]
    dev = contexts[data_len:], contexts_len[data_len:], pred[data_len:]

    config = Config()
    # config.strong_supervision = True
    model = ModelSentiment(config)

    model.set_data(train, dev, word_embedding, np.shape(contexts)[1])
    # model.set_encoding()
    model.init_ops()
